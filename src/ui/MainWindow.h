...
protected:
    ...
    /**
     * \brief Доразмещает боковую панель, когда у окна появилась настоящая ширина.
     *
     * Раскладка из конструктора считается по размеру по умолчанию, а не по
     * восстановленному, и запомненная ширина панели до первого показа не применяется.
     */
...
private:
    ...
    /**
     * \brief Разложить разделитель под текущее состояние боковой панели.
     *
     * Отдельно от setSidePanelExpanded(), потому что зовётся ещё и при первом показе
     * окна: до него у разделителя нет действительной ширины, и раскладка, посчитанная в
     * конструкторе, отношения к настоящему окну не имеет.
     *
     * \return `false`, если действительной ширины ещё нет и размеры не выставлялись;
     *         ограничения на ширину самой панели ставятся в любом случае.
     */
    bool applySidePanelGeometry();
     ...
     *
     * Нужен только когда основной сессии нет: в рабочем окне общим буфером служит
     * собственный буфер интерфейса A, чтобы история переживала смену режима.
     */
    ...
    /// \brief Раскладка боковой панели уже применена по настоящей ширине окна.
    bool m_sidePanelGeometryApplied = false;
    ...
    /// @{
    QLabel *m_stateIndicator = nullptr; ///< Значок состояния канала.
    QLabel *m_stateLabel = nullptr;     ///< Текстовое состояние канала.
