...
void TerminalView::scheduleMarkerUpdate()
{
    ...
    //
    // Проверка isActive() здесь обязательна, и это не оптимизация. QTimer::start() у
    // запущенного таймера отсчёт **перезапускает**: пока строки идут чаще, чем раз в
    // четверть секунды, срок всё время отодвигается, и пересчёт не случается ни разу.
    // Метки при этом появлялись только на остановленном выводе — то есть ровно тогда,
    // когда на них не смотрят. Нужно ограничение частоты, а не откладывание до затишья.
    if (m_markerTimer && !m_markerTimer->isActive())
        m_markerTimer->start();
}
