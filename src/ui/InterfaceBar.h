...
private:
    ...
    /**
     * \brief Пересчитать только время обнаружения у уже построенных пунктов.
     *
     * rebuild() зовётся по spotty::InterfaceRegistry::changed() — то есть когда список
     * устройств действительно изменился, а не когда истекла минута с последнего показа.
     * «N мин назад» без этого метода стояло бы неизменным между такими событиями и
     * прыгало бы сразу на верное значение при любой чужой правке (появление другого
     * устройства, смена псевдонима) — оба вида поведения читаются как «не работает».
     * Метод не трогает состав списка и не шлёт сигналов, поэтому безопасен и при открытом
     * выпадающем списке.
     */
    void refreshSecondaryTexts();
    ...
    /// \brief Тикает время обнаружения у уже построенных пунктов; см. refreshSecondaryTexts().
    QTimer *m_secondaryTextTimer = nullptr;
};
