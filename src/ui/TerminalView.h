...
public:
    ...
    /**
     * \brief Скрыть или показать строки одного транспорта.
     *
     * Фильтр показа, как #setCsvFilterEnabled и #setHideUnreadableEnabled: строка остаётся
     * в буфере со своей отметкой времени и достаётся плагинам, поиску и журналу. Панель,
     * разбирающая поток, продолжает считать скрытые строки — прячется ровно показ.
     *
     * Отправлять в скрытый транспорт по-прежнему можно: получатель выбирается в строке
     * отправки и от видимости не зависит. Скрытие говорит «не показывай мне это», а не
     * «отключи устройство».
     *
     * \note Системные сообщения не скрываются: «порт открыт», «устройство отключено» и
     *       ошибки нужны всегда — иначе фильтр прячет причину происходящего.
     */
    void setSourceVisible(quint8 source, bool visible);
    bool isSourceVisible(quint8 source) const;

    /// \brief Показать все транспорты. Зовётся при выходе из режима двух интерфейсов.
    void showAllSources();
...
Q_SIGNALS:
    ...
    /**
     * \brief Пользователь начал прокручивать содержимое.
     *
     * Владелец может ненадолго отложить фоновую работу, которая живёт в UI-потоке:
     * плавность непосредственного действия важнее опроса устройств в эту миллисекунду.
     */
    void scrollInteraction();
...
private:
    ...
    /**
     * \brief Провести список видимых в соответствие с буфером от #m_resolvedLine и далее.
     *
     * Строка, не завершённая переводом строки, может дописаться и сменить вердикт
     * фильтра — например, у неё появится вторая запятая, и она станет похожа на данные,
     * которых не было в первой пришедшей части. Только хвостовая строка бывает открытой
     * (новая появляется лишь после того, как закрылась предыдущая), поэтому пересмотр
     * стоит O(1): цикл останавливается на первой же незавершённой строке.
     */
    void syncVisible();
    ...
    /**
     * \brief Битовая маска скрытых транспортов: бит N — источник N.
     *
     * Маской, а не парой флагов: источник приходит числом из TerminalBuffer::Line, и
     * развилка «нулевой или первый» разошлась бы с ним, как только транспортов станет
     * больше двух.
     */
    quint32 m_hiddenSources = 0;
    ...
    /// \brief Первая строка, чей вердикт фильтра ещё не окончателен; см. #syncVisible.
    qint64 m_resolvedLine = 0;
