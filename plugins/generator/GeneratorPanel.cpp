...
{
    // Заголовок и поля раскладки даёт основа: они одинаковы у всех панелей окна.
    ...
    connect(m_toSendBar, &QPushButton::clicked, this, [this] {
        ...
        host()->composeInSendBar(DataCodec::toHex(m_generator.preview()),
...
void GeneratorPanel::sendOnce()
{
    ...
    host()->send(m_generator.generate());
