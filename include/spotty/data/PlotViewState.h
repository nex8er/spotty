...
public:
    ...
    /**
     * \brief Подвинуть окно за новыми данными.
     * \param first Время самого старого отсчёта.
     * \param last Время самого свежего.
     *
     * Зовётся при появлении отсчётов, а **не из отрисовки**: рисование не вправе менять
     * состояние — иначе каждый кадр просит следующий, и получается вечный цикл
     * перерисовки, который к тому же спорит с рукой пользователя.
     *
     * Пока идёт слежение, окно едет за \p last, сохраняя длительность. Иначе оно остаётся
     * там, куда его поставили: подтягивается лишь тогда, когда с данными не пересекается
     * вовсе и смотреть в нём заведомо нечего. Частичный выход за край допустим — без
     * этого перетаскивание у границы дёргалось бы.
     */
    void followTo(qint64 first, qint64 last);

    /**
     * \brief Вернуть слежение, если окно доведено до правого края.
     * \param last Время самого свежего отсчёта.
     *
     * Зовётся по завершении жеста — отпущенной кнопкой, концом прокрутки, — а не
     * непрерывно: проверка посреди перетаскивания включала бы слежение прямо под рукой, и
     * окно уезжало бы в конец, пока его тянут.
     */
    void snapToEnd(qint64 last);
