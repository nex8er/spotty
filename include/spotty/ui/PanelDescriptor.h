 ...
 *
 * Общий для строки отправки и IPanelHost::send(): панели, которые сами отправляют данные
 * (макросы, генератор), предлагают тот же выбор, что и строка отправки, а не свой.
 */
enum class SendTarget {
     ...
     *
     * Кто именно «первый», решает приложение: это состояние сессий, до которого ни строке
     * отправки, ни панели дела нет.
     */
    FirstAvailable,
};
